...
export class RoleService implements IRoleService {
  constructor(
    @inject(TOKENS.RoleRepository) private roleRepo: RoleRepository,
  ) {}

  async fetchRoleById(id: string): Promise<RoleResponseDto> {
    if (id) {
      throw new BadRequestException("Role name not provided");
    }
    const role = await this.roleRepo.fetchRoleById(id);
    if (!role?.roleId) {
      throw new InternalServerException("Failed to fetch role");
    }
