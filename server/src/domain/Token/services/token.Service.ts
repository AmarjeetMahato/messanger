...
export class TokenService implements ITokenService {
  ...
  async updateToken(tokenId: string, data: UpdateTokenInput): Promise<void> {
    ...
    const existingToken = await this.repo.fetchTokenById(tokenId);

    if (!existingToken) {
      throw new NotFoundExceptions("Token not found");
    }
    await this.repo.updateToken(tokenId, data);
  }
  ...
  async createToken(tokenData: CreateTokenInput): Promise<TokenResponseDto> {
    ...
    const token = await this.repo.fetchOTPwithUserId(
      tokenData.userId!,
      tokenData.type,
    );
    if (token?.tokenId) {
  ...
  async fetchOtp(userId: string, type: TokenType): Promise<TokenResponseDto> {
    if (!userId) {
      throw new BadRequestException("User is required");
    }
    const result = await this.repo.fetchOTPwithUserId(userId, type);
    if (!result?.tokenId) {
