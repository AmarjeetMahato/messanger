...
const Navbar = ({ conversation, chatId }: NavbarProps) => {
  ...
  const menuItems: MenuItem[] = [
    {
      ...
      label: "View contact",
      onClick: () => router.push(`/chat/${chatId}/view_contact`),
    },
    {
      icon: (
        <svg
    ...
    },
    {
      ...
      onClick: () => router.push(`/chat/${chatId}/media`),
    },
    {
      icon: (
        <svg
            ...
            <div className="flex items-center gap-1.5 mt-0.5">
              {conversation?.online ? (
                <>
                  ...
                  <span className="text-[12px] text-emerald-400/90 font-medium truncate">
                    Online
                  </span>
                </>
              ) : (
                <span className="text-[12px] text-white/30 truncate">
                  {formatWhatsAppTime(conversation?.time ?? "")}
                </span>
              )}
            </div>
          </div>
            ...
            {menuOpen && (
              <div className="absolute right-0 top-11 w-52 bg-[#13131f] border border-white/8 rounded-2xl shadow-2xl shadow-black/50 overflow-hidden z-50 backdrop-blur-xl">
                {menuItems.map((item, i) => {
                  ...
                  const { icon, label, danger } = item as {
                    icon: React.ReactNode;
                    label: string;
                    danger?: boolean;
                  };
                  return (
                    ...
                    >
                      <span
                        className={danger ? "text-red-400/70" : "text-white/35"}
                      >
                        {icon}
                      </span>
                      {label}
