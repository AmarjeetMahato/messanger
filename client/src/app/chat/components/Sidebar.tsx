...
} from "@/redux/rest-api/conversationApi";
...
export default function WhatsAppSidebar() {
  ...
  const { data, isLoading: convoLoading } = useFetchUserConversationQuery(
    userId ?? skipToken,
  );
  ...
  const filtered = contacts.filter((c) => {
    ...
    if (tab === "groups")
      return c.name.includes("Team") || c.name.includes("Crew");
    return true;
  });
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-[#0a0a0f] font-sans">
      <div
        className="relative flex flex-col w-full  h-180  overflow-hidden bg-[#0d001f] 
      shadow-[0_32px_80px_rgba(120,0,200,0.35),0_0_0_1px_rgba(150,80,220,0.2)]"
      >
        {/* Header Section */}
        <header className="relative p-5 pb-0 overflow-hidden bg-linear-to-br from-[#1a0030] via-[#2d0057] to-[#0d001a]">
          ...
          <div className="relative flex items-center gap-2.5 px-3.5 py-2.5 mb-4 bg-white/5 border border-purple-500/20 rounded-xl backdrop-blur-md transition-all focus-within:border-purple-500/40">
            ...
            {search && (
              <button
                onClick={() => setSearch("")}
                className="p-0.5 rounded-full bg-purple-500/20 text-purple-400 hover:bg-purple-500/40 transition-colors"
              >
                <X size={12} />
              </button>
            )}
          ...
          {tab === "people" && (
            <div className="flex flex-col" onScroll={handleScroll}>
              {alluserData?.data?.map((user: User) => (
                <div
                  key={user.id}
                ...
                >
                  ...
                  <div className="flex items-center gap-3">
                    {user.avatarUrl ? (
                      <div className="w-10 h-10 relative rounded-full overflow-hidden">
                        <Image
                          src={user?.avatarUrl || ""} // fallback avatar
                  ...
                  >
                    {user.id && <>{isLoading ? "Creating.." : "Chat"}</>}
                  </button>
                </div>
              ))}
            </div>
        ...
        {tab !== "people" && (
          <main className="flex-1 overflow-y-auto bg-linear-to-b from-[#0d001f] to-[#080010] [&::-webkit-scrollbar]:w-0.75 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-purple-900/60 [&::-webkit-scrollbar-thumb]:rounded-full">
            {filtered.length === 0 ? (
              <div className="text-center text-purple-400/40 py-10 text-[13px]">
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-center mb-0.5">
                      <h3
                        className={`text-[14px] truncate max-w-40 ${c.unread > 0 ? "font-semibold text-purple-50" : "font-normal text-purple-200"}`}
                      >
                        {c.name}
                      </h3>
                      <span
                        className={`text-[11px] ${c.unread > 0 ? "text-purple-500 font-semibold" : "text-purple-400/40"}`}
                      >
                    ...
                    <div className="flex justify-between items-center">
                      <p
                        className={`text-[12px] truncate max-w-47.5 ${c.unread > 0 ? "text-purple-400/80 font-medium" : "text-purple-400/50"}`}
                      >
                        {c.msg}
                      </p>
                      {c.unread > 0 && (
